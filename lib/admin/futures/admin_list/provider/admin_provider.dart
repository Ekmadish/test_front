...
class AdminProvider with ChangeNotifier implements BaseTableController {
  late LinkedScrollControllerGroup controllers;
  late ScrollController headController;
  late ScrollController bodyController;
  late List<AdminListDataList> _data = [];
  late List<AdminListDataList> currentData = [];
  ...
  @override
  void sort<T>(
    Comparable<T> Function(AdminListDataList d) getField,
    int columnIndex,
  ) {
  ...
  void _sort<T>(
      Comparable<T> Function(AdminListDataList d) getField, bool ascending) {
  ...
  void _nav(int index) async {
    ...
    currentData = d[index] as List<AdminListDataList>;
    notifyListeners();

    await onNavChages(index);
  }
  ...
  @override
  void lastPage() {
    currentData = _chunk(_data, perPageRow).last as List<AdminListDataList>;
  ...
  void setPerpage(int perPage) {
    ...
    var d = _chunk(_data, perPageRow);
    datalength = d.length;
    currentData = d[0] as List<AdminListDataList>;
    perPageRow = perPage;
    pageIndex = 0;
    notifyListeners();
  }
  ...
  @override
  void firstPage([List<AdminListDataList>? d]) {
    if (canFirst()) {
      currentData =
          _chunk(d ?? _data, perPageRow).first as List<AdminListDataList>;
  ...
  Future<void> onNavChages(int rowIndex) async {
    if (_pages.contains(rowIndex)) {
      ...
      if (!_isSarch) {
        await _getAdminList(length: _start + _start);
  ...
  // *--------the statelayer------
  void setState({required TableState s, List<AdminListDataList>? data}) {
    if (data != null) {
      print(data.first.toJson().toString());
      _data = data;
      log('the data length  ${_data.length}');
      _nav(0);
    }

    //* read data
    // var read = cacheClient.read('company');
    // userList.addAll(read);
  }

// *------------the datalayer--------
//meaning is searching now
  bool _isSarch = false;
  int _start = 0;
  ...
  void getFirstPage() async {
    await _getAdminList(length: 3, start: 0);
    _start = 0;
  }
  ...
  Future<void> _getAdminList(
    ...
    _repository
        .adminList(length: length ?? 0, start: start, keyWord: keyWord)
        .then((res) {
      if (keyWord!.isNotEmpty) {
        _searching(res);
      } else {
  ...
  void _searching(AdminListModel res) {
    if (res.total != 0) {
      setState(s: TableState.success(), data: res.dataList);
    }
  }
  ...
  Future<void> onPageChanged(int rowIndex) async {
    if (_pages.contains(rowIndex)) {
      ...
      if (!_isSarch) {
        _getAdminList(length: _start + _start);
        _pages.add((rowIndex.toDouble()));
      }
    }
  }
